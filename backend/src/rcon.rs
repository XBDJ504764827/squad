...
pub async fn validate_rcon_credentials(ip: &str, port: u16, password: &str) -> Result<(), String> {
    ...
    for _ in 0..2 {
        let (request_id, packet_type) = read_packet(&mut stream).await.map_err(map_io_error)?;
...
async fn read_packet(stream: &mut TcpStream) -> io::Result<(i32, i32)> {
